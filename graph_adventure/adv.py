...
# roomGraph={0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}], 3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [(3, 3), {'n': 5, 'w': 11}], 7: [(2, 5), {'w': 8, 'e': 0}], 8: [(1, 5), {'e': 7, 's': 9}], 9: [(1, 4), {'n': 8, 's': 10}], 10: [(1, 3), {'n': 9, 'e': 11}], 11: [(2, 3), {'w': 10, 'e': 6}]}
...
def traversal(player, graph):
  ...
  while rooms_to_visit:
    ...
    if c_rm_n and graph[c_rm.id][1]["n"] not in visited_rooms:
      ...
      player.travel('s')
      visited_rooms.add(graph[c_rm.id][1]['s'])
    elif c_rm_w and graph[c_rm.id][1]['w'] not in visited_rooms:
