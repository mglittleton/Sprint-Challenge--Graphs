...
def nearest_open_path(start, graph, visited):
  ...
  while not found_path:
      ...
      if curr_room not in visited_paths:
          ...
          for d in curr_room[1]:
            if curr_room[1][d] in visited:
              new_path = {"room": graph[curr_room[1][d]], "path": list(curr_path["path"])}
              new_path["path"].append(d)
              queue.enqueue(new_path)
            else:
              curr_path["path"].append(d)
              return {"room": curr_room[1][d], "path": curr_path["path"]}
          # for vert in self.vertices[curr_room]:
...
def traversal(player, graph):
  ...
  while rooms_to_visit:
    c_rm = player.currentRoom
    c_rm_n = c_rm.n_to
    c_rm_s = c_rm.s_to
    c_rm_e = c_rm.e_to
    c_rm_w = c_rm.w_to

    # possible info to use
    # print("can go", graph[c_rm.id][1])
    # print("get exits", c_rm.getExits())
    # if c_rm_n:
    #   print ("can go north to")
    #   print (graph[c_rm.id][1])
    #   print (graph[c_rm.id][1]["n"])

    if c_rm_n and graph[c_rm.id][1]["n"] not in visited_rooms:
      traversalPath.append('n')
      player.travel('n')
      visited_rooms.add(graph[c_rm.id][1]['n'])
      print("n")
    elif c_rm_e and graph[c_rm.id][1]['e'] not in visited_rooms:
      traversalPath.append('e')
      player.travel('e')
      visited_rooms.add(graph[c_rm.id][1]['e'])
      print("e")
    elif c_rm_s and graph[c_rm.id][1]['s'] not in visited_rooms:
      traversalPath.append('s')
      player.travel('s')
      visited_rooms.add(graph[c_rm.id]['s'])
      print("s")
    elif c_rm_w and graph[c_rm.id][1][1]['w'] not in visited_rooms:
      traversalPath.append('w')
      player.travel('w')
      visited_rooms.add(graph[c_rm.id][1]['w'])
      print("w")
    else:
      last_open = nearest_open_path(graph[c_rm.id], graph, visited_rooms)
      print(last_open)
